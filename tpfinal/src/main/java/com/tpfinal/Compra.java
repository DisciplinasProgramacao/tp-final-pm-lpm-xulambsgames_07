...
public class Compra {
  ...
  Compra(LocalDate data, Cliente cliente, ArrayList<Jogo> itensCompra) {
    this.setData(data);
  ...
  private double calcularPorcentagemDesconto(int[] relacaoPorTipoDeJogo) {
    ...
    Boolean condicaoBase = relacaoPorTipoDeJogo[0] > 2 || (relacaoPorTipoDeJogo[1] == 2 && totalDeJogos >= 3) || (relacaoPorTipoDeJogo[1] == 3);
  ...
  public String toString() {
    return "Cliente: " + this.cliente.getNome() + " > jogos: " + Arrays.toString(this.itensCompra.stream().map((c) -> c.getNome()).toArray()) + " | valor base: " + this.precoBase + " | valor após descontos: " + this.getValorTotal();
  }
}